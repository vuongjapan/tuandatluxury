...
function extractIframeSrc(html: string): string {
  if (!html) return DEFAULT_MAP_SRC;
...
const MapSection = () => {
  ...
  const mapSrc = embedCode ? extractIframeSrc(embedCode) : DEFAULT_MAP_SRC;
    ...
    <section id="map" className="py-10 sm:py-16 bg-secondary">
      <div className="section-container">
        ...
        <div className="rounded-2xl overflow-hidden border border-border shadow-card bg-card">
          <iframe
            src={mapSrc}
            className="w-full h-[250px] sm:h-[350px] md:h-[450px] lg:h-[500px] border-0"
            allowFullScreen
            loading="lazy"
